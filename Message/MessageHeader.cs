...
public readonly struct MessageHeader(int length, ushort id)
{
    ...
    public static MessageHeader Decode(ReadOnlySpan<byte> buffer)
    {
        if (buffer.Length < Size)
            throw new Exception($"Header buffer must be at least {Size} bytes long");
        ...
        return new MessageHeader(length, id);
    }
}
